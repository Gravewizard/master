...
<table id="myTable1">
  <tr class="header">
    ...
    <th style="width:50%;">Option</th>
                                                ...
                                                <?php
					                        		$product_array = "SELECT * FROM renewroom where payment=0";
							                        $rs = mysqli_query($link,$product_array);
							                        while($row = mysqli_fetch_array($rs)) {
					                            ?>
  <tr>
    <td><?php echo $row['o_ownername']; ?></td>
    <td><a href="rroompay.php?id=<?php echo $row['id']; ?>"><button type="button" class="btn btn-success">Pay Amount</button></a></td>
  </tr>
  <?php } ?>
